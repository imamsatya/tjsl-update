                        ...
                        <table class="table table-striped table-bordered table-hover tree  table-checkable">
                            ...
                            @foreach ($bumn as $b)     
                                @php 
                                    ...
                                    $isCompanyExist = $anggaran->where('perusahaan_id', $b->id);
                                    if($isCompanyExist->count()) {
                                ...
                                <tr class="treegrid-bumn{{@$b->id}}" >
                                    ...
                                    <td style="display: flex"><div style="flex: 1">{{$b->nama_lengkap}}</div></td>
                                    ...
                                    <td style="text-align:center;">
                                        <a class="badge badge-light-{{$status_class}} fw-bolder me-auto px-4 py-3" data-toggle="tooltip" title="Lihat Log">{{ $isCompanyExist->count() ? @$statusPerusahaan->status->nama : '-'}}</a>
                                    </td>
                                    <td></td>
                                ...
                                @foreach ($anggaran_pilar_bumn as $p)                              
                                    ...
                                    <tr class="treegrid-bumn{{@$b->id}}pilar{{str_replace(' ', '-', @$p->pilar_nama)}} {{$class_parent}} item-bumn{{@$b->id}}pilar{{str_replace(' ', '-', @$p->pilar_nama)}}" >
                                        ...
                                        <td style="display: flex"><div style="flex: 1">{{$p->pilar_nama}}</div></td>
                                    ...
                                    <tr class="treegrid-{{$a->id}} treegrid-parent-bumn{{@$b->id}}pilar{{str_replace(' ', '-', @$p->pilar_nama)}} item{{$a->id}}">
                                        <td></td>
                                        <td style="display: flex"><div style="margin-left: 5%; flex: 1">{{@$a->no_tpb .' - '. @$a->tpb_nama}}</div></td>
                                        ...
                                        <td style="text-align:center;">
                                            @if(!$view_only)
                                                @if($status_id != 1)
                                                <button {{ $isOkToInput || $a->is_enable_input_by_superadmin ? '' : 'disabled' }} type="button" class="btn btn-sm btn-light btn-icon btn-primary cls-button-edit" data-anggaran-cid="{{ $id_anggaran_cid }}" data-anggaran-noncid="{{ $id_anggaran_noncid }}" data-id="{{$a->id}}" data-toggle="tooltip" title="Ubah data {{@$a->no_tpb}}"><i class="bi bi-pencil fs-3"></i></button>
