                ...
                <div class="card-header pt-5">
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex align-items-center position-relative my-1"
                            ...
                            <button type="button" class="btn btn-success btn-sm cls-add" data-toggle="tooltip"
                                ...
                                <thead>
                                    <tr>
                                        <th
                                            ...
                                            No.</th>
                                        <th style="font-weight:bold;border-bottom: 1px solid #c8c7c7;">Versi - Pilar - TPB
                                        </th>
                                        <th
                                        ...
                                        <th
                                            ...
                                            Status</th>
                                        <th
                                            style="text-align:center;width:120px;font-weight:bold;border-bottom: 1px solid #c8c7c7;">
                                            Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    @foreach ($versi as $p)
                                        <tr class="treegrid-versi{{ @$p->id }}">
                                            ...
                                            <td style="text-align:center;">
                                                <button type="button" data-id="{{ $p->id }}"
                                                    class="btn btn-sm btn-light btn-icon btn-success cls-button-add-pilar"
                                                    data-id="{{ $p->id }}" data-toggle="tooltip"
                                                    title="Tamba data Pilar"><i class="bi bi-plus fs-3"></i></button>
                                                <button type="button"
                                                    class="btn btn-sm btn-light btn-icon btn-primary cls-button-edit"
                                                    data-id="{{ $p->id }}" data-toggle="tooltip"
                                                    title="Ubah data versi {{ @$p->versi }}"><i
                                                        class="bi bi-pencil fs-3"></i></button>
                                                <button type="button"
                                                    class="btn btn-sm btn-danger btn-icon cls-button-delete"
                                                    data-id="{{ $p->id }}" data-nama="Versi {{ $p->versi }}"
                                                    data-toggle="tooltip" title="Hapus data Versi {{ $p->versi }}"><i
                                        ...
                                        @foreach ($pilar as $a)
                                            <tr
                                                class="treegrid-pilar{{ $a->id }}versi{{ @$p->id }} treegrid-parent-versi{{ @$p->id }} item-pilar{{ $a->id }}versi{{ @$p->id }}">
                                                <td></td>
                                                <td colspan="5">{{ $a->nama }} - {{ $a->jenis_anggaran }}</td>
                                                <td style="text-align:center;">
                                                    <button type="button" data-id="{{ $a->id }}"
                                                        data-versi="{{ $p->id }}"
                                                        class="btn btn-sm btn-light btn-icon btn-primary cls-button-edit-pilar"
                                                        data-id="{{ $a->id }}" data-toggle="tooltip"
                                                        title="Ubah data {{ @$a->nama }}"><i
                                                            class="bi bi-pencil fs-3"></i></button>
                                                    <button type="button" data-id="{{ $a->id }}"
                                                        data-versi="{{ $p->id }}"
                                                        class="btn btn-sm btn-danger btn-icon cls-button-delete-pilar"
                                                        data-id="{{ $a->id }}" data-nama="{{ $a->nama }}"
                                                        data-toggle="tooltip" title="Hapus data {{ $a->nama }}"><i
                                                            class="bi bi-trash fs-3"></i></button>
                                                </td>
                                            </tr>
                                            ...
                                            @foreach ($tpb as $c)
                                                <tr
                                                    class="treegrid-tpb{{ $c->id }} treegrid-parent-pilar{{ @$a->id }}versi{{ @$p->id }} item-tpb{{ $c->id }}">
                                                    <td></td>
                                                    ...
                                                    <td style="text-align:center;">
                                                        <button type="button" data-id="{{ $c->id }}"
                                                            ...
                                                            data-pilar="{{ $a->id }}"
                                                            data-versi="{{ $p->id }}"
                                                            class="btn btn-sm btn-light btn-icon btn-primary cls-button-edit-tpb"
                                                            data-id="{{ $c->id }}" data-toggle="tooltip"
                                                            title="Ubah data {{ @$c->no_tpb }}"><i
                                                                class="bi bi-pencil fs-3"></i></button>
                                                    </td>
                                                </tr>
                                            @endforeach
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '.cls-button-edit-pilar', function() {
                winform(urleditpilar, {
                    'id': $(this).data('id'),
                    'versi': $(this).data('versi')
                }, 'Ubah Data');
            });
