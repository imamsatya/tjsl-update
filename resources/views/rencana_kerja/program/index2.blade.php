                            ...
                            <table class="table table-striped table-bordered table-hover tree-new  table-checkable">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align:center;font-weight:bold;border-bottom: 1px solid #c8c7c7;"> Anggaran </th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="treegrid-perusahaan-{{ $perusahaan[0]->perusahaan_id }}" id="perusahaan-{{ $perusahaan[0]->perusahaan_id }}" data-type="perusahaan" data-value="{{ $perusahaan[0]->perusahaan_id }}">
                                        ...
                                        <td style="text-align: right; white-space: nowrap;">
                                            ...
                                            @php
                                                $color_status = '';
                                                if($perusahaan[0]->total < $perusahaan[1]->total_rka) $color_status = 'orange';
                                                else if($perusahaan[0]->total > $perusahaan[1]->total_rka) $color_status = 'red';
                                                else $color_status = 'green';
                                            @endphp
                                            <span style="color: {{ $color_status }}; font-size: smaller">RKA: {{number_format($perusahaan[1]->total_rka,0,',',',')}}</span>
                                        </td>
                                        <td></td>
                                        <td style="text-align: center">
                                            <a class="badge badge-light-{{ $status_class }} fw-bolder me-auto px-4 py-3" data-toggle="tooltip" title="Lihat Log">{{ $perusahaan[0]->inprogress ? 'In Progress' : ($perusahaan[0]->completed ? 'Verified' : ($perusahaan[0]->verified ? 'Validated' : 'Unfilled') ) }}</a>
                                        </td>
                                        <td></td>
                                    </tr>
        ...
        $(document).ready(function() {
                        ...
                        for(let i=0; i < pilar.length; i++) {
                            ...
                            if(pilar[i][0]) {
                                ...
                                <tr class="treegrid-bumn-${value}-pilar-${pilar[i][0].id_pilar} ${parentClass}" data-type="pilar" data-value="${pilar[i][0].id_pilar}" data-perusahaan="${value}" id="perusahaan-${value}-pilar-${pilar[i][0].id_pilar}">
                                    ...
                                    <td></td>
                                    <td style="text-align:center;">
                                        <a class="badge badge-light-${status_class} fw-bolder me-auto px-4 py-3" data-toggle="tooltip" title="Lihat Log">${pilar[i][0].inprogress ? 'In Progress' : (pilar[i][0].completed ? 'Verified' : (pilar[i][0].verified ? 'Validated' : 'Unfilled'))}</a>
                                    </td>
                                    <td style="text-align:center;">                                            
                                    </td>                                
                        ...
                        for(let i=0; i<tpb.length; i++) {
                            ...
                            if(tpb[i][0]) {
                                ...
                                <tr class="treegrid-bumn-${tempPerusahaan}-pilar-${value}-tpb-${tpb[i][0].id_tpb} ${parentClass}" data-type="tpb" data-value="${tpb[i][0].id_tpb}" data-perusahaan="${tempPerusahaan}" data-pilar="${value}" id="perusahaan-${tempPerusahaan}-pilar-${value}-tpb-${tpb[i][0].id_tpb}">
                                    ...
                                    <td></td>
                                    <td style="text-align:center;">
                                        <a class="badge badge-light-${status_class} fw-bolder me-auto px-4 py-3" data-toggle="tooltip" title="Lihat Log">${tpb[i][0].inprogress ? 'In Progress' : (tpb[i][0].completed ? 'Verified' : (tpb[i][0].verified ? 'Validated': 'Unfilled'))}</a>
                                    </td>
                                    <td style="text-align:center;">                                   
                                    </td> 
                                    <td><label class="form-check form-check-sm form-check-custom form-check-solid me-5 me-lg-20 mt-3">
                    ...
                    if(!dataTpbLoaded) {
                        ...
                        for(let i=0; i<target.length; i++) {
                            ...
                            <tr class="${parentClass}" data-type="target" data-value="${target[i].id_target}" data-perusahaan="${tempPerusahaan}" data-pilar="${tempPilar}" data-tpb="${value}">
                                ...
                                <td style="text-align:center;">
                                    <span class="btn cls-log badge badge-light-${status_class} fw-bolder me-auto px-4 py-3" data-id="${target[i].id_target}">${target[i].inprogress ? 'In Progress' : (target[i].completed ? 'Verified' : (target[i].verified ? 'Validated' : '-'))}</span>
                                </td>
                                <td style="text-align:center;">    
            ...
            $("#completed-data").on('click', function() {
                if(!countInprogress) {
                    swal.fire({
                        title: "Pemberitahuan",
                        html: "Tidak ada data yang bisa di set verified!",
                        icon: "warning",
                ...
                swal.fire({
                    title: "Pemberitahuan",
                    html: `<span style="color: red; font-weight: bold">Yakin set data verified ? </span><br/>
                        <div class="table-responsive">
            ...
            $("#uncompleted-data").on('click', function() {
                if(!countCompleted) {
                    swal.fire({
                        title: "Pemberitahuan",
                        html: "Tidak ada data yang bisa di-set unverified!",
                        icon: "warning",
                        showCancelButton: false,
                        confirmButtonText: "Close",
                    })
                ...
                swal.fire({
                    title: "Pemberitahuan",
                    html: `<span style="color: red; font-weight: bold">Yakin set data unverified ? </span><br/>
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped">
                                <tbody>
                                    <tr class="fw-bold fs-6 text-gray-800" style="text-align: left">
            ...
            $("#verify-data").on('click', function() {
                if(!countInprogress) {
                    swal.fire({
                        title: "Pemberitahuan",
                        html: "Tidak ada data yang bisa di-validate!",
                        icon: "warning",
                        showCancelButton: false,
                        confirmButtonText: "Close",
                    })
            ...
            $("#unverify-data").on('click', function() {  
                ...
                if(!countVerified) {
                    swal.fire({
                        title: "Pemberitahuan",
                        html: "Tidak ada data yang bisa di unvalidate!",
                        icon: "warning",
                        showCancelButton: false,
                        confirmButtonText: "Close",
                    })
