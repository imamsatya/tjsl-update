<form class="kt-form kt-form--label-right" method="POST" id="form-edit">
    ...
    @if (!$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin)
        ...
        <div class="alert alert-danger d-flex align-items-center p-5"
            style="    border-radius: 0.5em;background-color: #fff5f8;color: #f1416c;border-color: #f1416c">
            <!--begin::Icon-->
            <i class=" bi-shield-fill-x fs-2hx text-danger me-4"><span class="path1"></span><span
                    class="path2"></span></i>
            <!--end::Icon-->
    ...
    <div class="mb-6 ">
        <div class="row mb-6">
            ...
            <div class="col-lg-9">
                <select {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                    id="tpb_id_edit" class="form-select form-select-solid form-select2" name="tpb_id_edit"
                    data-kt-select2="true" data-placeholder="Pilih TPB" data-allow-clear="true">
                    <option></option>
                    @foreach ($tpb as $p)
                        <option {{ $data->tpb_id === $p->id ? "selected='selected'" : '' }} value="{{ $p->id }}">
                            {{ $p->no_tpb }} - {{ $p->nama }} [{{ $p->jenis_anggaran }}]</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="row mb-6">
            ...
            <div class="col-lg-9">
                <textarea {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }} class="form-control"
                    id="nama_program_edit" name="nama_program_edit" style="height: 100px">{{ $data->program }}</textarea>
            </div>
        </div>
        <div class="row mb-6">
            <div class="col-lg-3">
            ...
            <div class="col-lg-9">
                <input {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                    value="{{ $data->unit_owner }}" type="text" name="unit_owner_edit" id="unit_owner_edit"
                    class="form-control form-control-lg form-control-solid" placeholder="Unit Owner" />
            </div>
        </div>
        <div class="row mb-6">
            <div class="col-lg-3">
            ...
            <div class="col-lg-9 fv-row d-flex align-items-center justify-content-start">
                <div style="display:flex; flex-direction: row;">
                    <div class="form-check form-check-custom form-check-solid form-check-sm me-8">
                        <input {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                            {{ $data->kriteria_program_prioritas ? 'checked="checked"' : '' }} class="form-check-input"
                            type="checkbox" name="kriteria_program_edit" value="prioritas"
                            id="checkboxPrioritas_edit" />
                    ...
                    <div class="form-check form-check-custom form-check-solid form-check-sm me-8">
                        <input {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                            {{ $data->kriteria_program_csv ? 'checked="checked"' : '' }} class="form-check-input"
                            type="checkbox" name="kriteria_program_edit" value="csv" id="checkboxCSV_edit" />
                    ...
                    <div class="form-check form-check-custom form-check-solid form-check-sm">
                        <input {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                            {{ $data->kriteria_program_umum ? 'checked="checked"' : '' }} class="form-check-input"
                            type="checkbox" name="kriteria_program_edit" value="umum" id="checkboxUmum_edit" />
        ...
        <div class="row mb-6">
            ...
            <div class="col-lg-9">
                <select {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                    id="core_subject_id_edit" class="form-select form-select-solid form-select2"
                    name="core_subject_id_edit" data-kt-select2="true" data-placeholder="Pilih ID Core Subject"
                    data-allow-clear="true">
                    <option></option>
                    @foreach ($core_subject as $c)
                        <option {{ $data->core_subject_id === $c->id ? 'selected="selected"' : '' }}
                            value="{{ $c->id }}">{{ $c->nama }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row mb-6">
            ...
            <div class="col-lg-9">
                <select {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                    id="pelaksanaan_program_edit" class="form-select form-select-solid form-select2"
                    name="pelaksanaan_program_edit" data-kt-select2="true"
                    data-placeholder="Pilih Pelaksanaan Program" data-allow-clear="true">
                    <option></option>
                    <option value="Mandiri"
                        {{ $data->pelaksanaan_program === 'Mandiri' ? 'selected="selected"' : '' }}>
                        Mandiri</option>
                    <option value="Kolaborasi"
                        {{ $data->pelaksanaan_program === 'Kolaborasi' ? 'selected="selected"' : '' }}>
                        Kolaborasi</option>
                </select>
            </div>
        ...
        <div class="row mb-6">
            ...
            <div class="col-lg-9">
                <select {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                    class="form-select form-select-solid form-select2" id="mitra_bumn_edit" name="mitra_bumn_edit"
                    data-kt-select2="true" data-placeholder="Pilih Mitra BUMN" data-allow-clear="true">
                    <option></option>
                    @foreach ($perusahaan as $p)
                        <option {{ $data->mitra_bumn_id === $p->id ? 'selected="selected"' : '' }}
                            value="{{ $p->id }}">{{ $p->nama_lengkap }}</option>
                    @endforeach
                </select>
            </div>
        ...
        {{-- <div class="row mb-6">
            ...
            <div class="col-lg-9 fv-row d-flex align-items-center justify-content-start">
                <div style="display:flex; flex-direction: row;">
                    <div class="form-check form-check-custom form-check-solid form-check-sm me-8">
                        <input {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                            {{ $data->multi_years ? 'checked="checked"' : '' }} class="form-check-input"
                            type="radio" name="program_edit" id="multiyears_ya_edit" value="ya" />
                    ...
                    <div class="form-check form-check-custom form-check-solid form-check-sm me-8">
                        <input {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                            {{ !$data->multi_years ? 'checked="checked"' : '' }} class="form-check-input"
                            type="radio" name="program_edit" id="multiyears_tidak_edit" value="tidak" />
        ...
        <div class="row mb-6">
            ...
            <div class="col-lg-9">
                <input {{ !$isOkToInput && !$isEnableInputBySuperadmin && !$isSuperAdmin ? 'disabled' : '' }}
                    type="text" name="alokasi_anggaran_edit" id="alokasi_anggaran_edit"
                    class="form-control form-control-lg form-control-solid" placeholder="Rp ... "
                    oninput="formatCurrency2(this)" onkeypress="return onlyNumbers(event)" style="text-align:right;"
                    value="{{ number_format($data->anggaran_alokasi, 0, ',', ',') }}" />
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#form-edit').on('submit', function(event) {
            ...
            if (!data.nama_program_edit.length || !data.tpb_id_edit.length || !data.kriteria_used
                .length || !data.core_subject_id_edit.length || !data.pelaksanaan_program_edit.length ||
                !data.alokasi_anggaran_edit.length) {
                swal.fire({
                    icon: 'warning',
                    title: 'Warning',
