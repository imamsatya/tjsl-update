    ...
    public function collection(Collection $row)
    {
       ...
       foreach ($row as $ar) {
        ...
        //validasi data
            ...
            // cek no ktp
            try{
                $no_id = strlen(preg_replace('/[^0-9]/','',$ar['nik'])) == 16? substr($ar['nik'],-4) == 0000? false : true : false;
            ...
            // cek sektor
            try{
                $sektors = rtrim($ar['sektor_usaha'])? true : false;
                $is_angka = is_numeric($ar['sektor_usaha'])? true : false;

                if($sektors && $is_angka){
                    $sektor = SektorUsaha::find(rtrim($ar['sektor_usaha']));
                }else{
                    $ar['sektor_usaha'];
                }   
            ...
            // cek provinsi 
                try{
                    $prov = rtrim($ar['provinsi'])? true : false;
                    $is_angka = is_numeric($ar['provinsi'])? true : false;

                    if($prov && $is_angka){
                        $provinsi = Provinsi::find(rtrim($ar['provinsi']));
                    }else{
                        $ar['provinsi'];
                    }
            ...
            // cek kota
                try{
                    $kotas = rtrim($ar['kabupatenkota'])? true : false;
                    $is_angka = is_numeric($ar['kabupatenkota'])? true : false;

                    if($kotas && $is_angka){
                        $kota = Kota::find(rtrim($ar['kabupatenkota']));
                    }else{
                        $ar['kabupatenkota'];
                    }                    
            ...
            // cek relasi provinsi kota
                try{
                    $kota = Kota::where('id',rtrim($ar['kabupatenkota']))
                                ->where('provinsi_id',rtrim($ar['provinsi']))
                                ->first();
                    if(!$kota){
                        // DB::rollback();
                        $is_gagal = true;
            ...
            // cek tgl awal
            try{
                $tglawal = $ar['tanggal_mendapatkan_pendanaan'] !== null? true : false;
                
                if(!$tglawal){
                    // DB::rollback();
                    $is_gagal = true;
                    $keterangan .= 'Baris '.rtrim($ar['no']).' Tanggal Awal Pendanaan Kosong.<br>';
                }else if(!is_numeric($ar['tanggal_mendapatkan_pendanaan'])){
                    // DB::rollback();
                    $ar['tanggal_mendapatkan_pendanaan'];
                    $is_gagal = true;
            ...
            // tenor
            try {
                $tenor = rtrim($ar['tenor_bulan']) ? true: false;
                $is_angka = is_numeric($ar['tenor_bulan'])? true : false;
            ...
            try{
                $params = rtrim($ar['outstanding_pokok'])? true : false;
                if($params){
                    $ar['outstanding_pokok'];
                    $cek = true;
                }else{
                    $ar['outstanding_pokok'] = 0;
                    $cek = false;
                }
            }catch(\Exception $e){
            }
        ...
        // cek saldo jasa adm Pendanaan
            try{
                $params = rtrim($ar['outstanding_jasa_admin'])? true : false;
                if($params){
                    $ar['outstanding_jasa_admin'];
                    $cek = true;
                }else{
                    $ar['outstanding_jasa_admin'] = 0;
                    $cek = false;
                }
            }catch(\Exception $e){
            }
         ...
         // cek Kolektibilitas
         try{
            $params = rtrim($ar['kolektibilitas'])? true : false;
            $is_angka = is_numeric($ar['kolektibilitas'])? true : false;

            if($params && $is_angka){
                $kolek = KolekbilitasPendanaan::find(rtrim($ar['kolektibilitas']));
            }else{
                $ar['kolektibilitas'];
            }   
        ...
        // cek KondisiPinjaman
        try{
            $params = rtrim($ar['kondisi_pinjaman_belumtelah_di_restruktur'])? true : false;
            $is_angka = is_numeric($ar['kondisi_pinjaman_belumtelah_di_restruktur'])? true : false;

            if($params && $is_angka){
                $kondisi = KondisiPinjaman::find(rtrim($ar['kondisi_pinjaman_belumtelah_di_restruktur']));
            }else{
                $ar['kondisi_pinjaman_belumtelah_di_restruktur'];
            } 
        ...
        if(!rtrim($ar['bumn_sumber_dana']) && !is_numeric($ar['bumn_sumber_dana'])) {
            // DB::rollback();
            $is_gagal = true;
            $keterangan .= 'Baris '.rtrim($ar['no']).' Sumber Dana Wajib Diisi.<br>';
        }
        ...
        $exist_message = $exist_update ? $exist_update.' data sudah ada dan sama dengan data sebelumnya di periode dan tahun yang sama' : '';
        $gagal_message = $gagal ? $gagal.' data gagal upload, detail: '.$keterangan : '';
        $total_notupdate = $gagal + $exist_update;
        ...
        $param['gagal'] = $total_notupdate;
        $param['keterangan'] = $exist_update ? $exist_message.' - '.$gagal_message : $gagal_message;
