...
class DownloadKegiatan implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($data, $part, $downloadId, $tahun)
    {
        //
        $this->data = $data;
        $this->part = $part;
    ...
    public function handle()
    {
        ...
        $kegiatan = $this->data;
        $namaFile = "Kegiatan ".date('dmY')." ".($this->part).".xlsx";
        // return Excel::download(new KegiatanBulanExport($kegiatan, $request['tahun), $namaFile);   

        Excel::store(new KegiatanBulanExport($kegiatan, $tahun), 'public/download_kegiatan/'.$namaFile);
